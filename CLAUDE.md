...
Modelled on the HALBE magnet frame range (halbe-rahmen.de). Per instance: `frameStyle` (`"<profile>-<finish>"`, e.g. `alu8-silber-matt`, or `'none'`), `passepartoutWidth` (cm at the sides, 0 = none) and `passepartoutPlacement` (`center` | `optical-center` | `golden-ratio`). One catalogue, `src/lib/frameStyles.ts`, is the single source of truth; `server/src/lib/frameStyles.ts` mirrors the profile → finish table for API validation (and maps the first catalogue's legacy ids like `alu-silver`, so does `frameStyleOf` on the client).

- Catalogue: 10 profiles (Alu 6/7/8/12/14/18, Holz 10/16/20/22) with HALBE's face width and depth from their cross-section drawings, 16 finishes, and only the combinations HALBE sells (`PROFILE_FINISHES`, 57 styles). The frame opening is always exactly the picture (or passepartout) — no rebate covers it.
...
- Textures (`src/lib/frameTextures.ts`, pure functions): wood grain (fine wandering lines, oak pores, walnut streaks) and brushed steel, generated in `workers/frameTexture.worker.ts` (~100 ms per finish). `frameMaterials.getFrameMaterial` returns the material at once in the finish's average colour and attaches the textures when they arrive (`onFrameTexturesReady` → FrameInstancer invalidates). No canvas, no `onBeforeCompile` — WebGPU converts these materials to NodeMaterials and would drop a shader patch.
- Colours: wood colours are albedos calibrated so the frame renders like HALBE's product photos under the gallery lighting + ACES. `THREE.Color('#hex')` converts to linear — build byte data from the hex directly (`rgbOf`), not from a Color.
